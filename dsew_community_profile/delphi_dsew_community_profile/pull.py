...
RE_DATE_FROM_HOSP_HEADER = re.compile(rf"HOSPITAL UTILIZATION: (.*) WEEK \({DATE_RANGE_EXP}\)")
...
class DatasetTimes:
    ...
    def from_header(header, publish_date):
        ...
        return DatasetTimes(
            column,
            positivity_reference_date,
            total_reference_date,
            hosp_reference_date,
            cumulative_vac_reference_date,
            vac_reference_date,
        )
    ...
    def __getitem__(self, key):
        ...
        raise ValueError(
            f"Bad reference date type request '{key}'; " + "need one of: " + " ,".join(ref_list)
        )
    ...
    def __eq__(self, other):
        ...
        return (
            isinstance(other, DatasetTimes)
            and other.column == self.column
            and other.positivity_reference_date == self.positivity_reference_date
            and other.total_reference_date == self.total_reference_date
        )
...
class Dataset:
    ...
    def __init__(self, config, sheets=TRANSFORMS.keys(), logger=None):
        ...
        self.url = DOWNLOAD_ATTACHMENT.format(
            assetId=config["assetId"], filename=quote_as_url(config["filename"])
        )
    ...
    def parse_publish_date(report_filename):
        ...
        return datetime.date(*[int(x) for x in RE_DATE_FROM_FILENAME.findall(report_filename)[0]])
    ...
    def _parse_times_for_sheet(self, sheet):
        ...
        overheaders = (
            pd.read_excel(self.workbook, sheet_name=sheet.name, header=None, nrows=1)
            .values.flatten()
            .tolist()
        )
    ...
    def retain_header(header):
        ...
        return (
            all([
                ...
                (
                    header.startswith("Total NAATs")
                    or header.startswith("NAAT positivity rate")
                    or header.startswith("Total RT-PCR")
                    or header.startswith("Viral (RT-PCR)")
                    or header.startswith("Booster")
                    or header.startswith("Doses administered -")
                ),
            ...
            or all([
                ...
                # exclude "People who have received a booster dose as % of fully vaccinated" ...
                header.find("%") < 0,
    ...
    def _parse_sheet(self, sheet):
        ...
        select = [select_fn(h) for h in list(df.columns) if self.retain_header(h)]

        for sig in SIGNALS:
            ## Check if field is known to be missing
            # Hospital admissions not available at the county or CBSA level prior to Jan 8, 2021.
            is_hosp_adm_before_jan8 = (
                (sheet.level == "msa" or sheet.level == "county")
            ...
            is_booster_before_nov1 = self.publish_date < datetime.date(2021, 11, 1) and (
                sig in ["booster dose since", "booster doses administered"]
            )
            # Booster and weekly doses administered not available below the state level.
            is_booster_below_state = (sheet.level != "hhs" and sheet.level != "state") and (
                sig
                in [
                    "doses administered",
            ...
            is_dose_admin_apr29 = (
                self.publish_date <= datetime.date(2021, 4, 29) and sig == "doses administered"
            )
            ...
            is_fully_vax_before_jan14 = (
                self.publish_date <= datetime.date(2021, 1, 14) and sig == "fully vaccinated"
            )
...
def fetch_listing(params):
    ...
    if params["indicator"]["reports"] == "new":
        ...
        keep = [el for el in listing if start_date <= el["publish_date"] <= end_date]
    ...
    if "export_start_date" in params["indicator"]:
        keep = [
            el for el in keep if params["indicator"]["export_start_date"] <= el["publish_date"]
        ]
...
def nation_from_state(df, sig, geomapper):
    ...
    if SIGNALS[sig]["is_rate"]:  # true if sig is a rate
        df = geomapper.add_population_column(df, "state_id").rename(
            columns={"population": "weight"}
        )

        norm_denom = df.groupby("timestamp").agg(norm_denom=("weight", "sum"))
        df = (
            df.join(norm_denom, on="timestamp", how="left")
            .assign(weight=lambda x: x.weight / x.norm_denom)
            .drop("norm_denom", axis=1)
        )
    ...
    publish_date_by_ts = df.groupby(["timestamp"])["publish_date"].first().reset_index()
...
def keep_latest_report(df, sig):
    ...
    df = (
        ...
        .apply(lambda x: x[x["publish_date"] == x["publish_date"].max()])
        .drop_duplicates()
    )
    ...
    if not df.empty:
        ...
        assert (
            all(df.groupby(["timestamp", "geo_id"]).size().reset_index(drop=True) == 1)
        ), (
            f"Duplicate rows in {sig} indicate that one or"
...
def fetch_new_reports(params, logger=None):
    ...
    for key, df in ret.copy().items():
        ...
        if sig == "positivity":
            ...
            ret[key] = unify_testing_sigs(df, ret[total_key]).drop("publish_date", axis=1)
...
def generate_prop_signal(df, geo, geo_mapper):
    ...
    if geo == "msa":
        ...
        map_df = (
            geo_mapper.add_population_column(map_df, "fips")
            .drop("fips", axis=1)
            .groupby(geo)
            .sum()
            .reset_index()
...
def unify_testing_sigs(positivity_df, volume_df):
    ...
    pos_count_ts = (
        positivity_df.groupby(["publish_date", "geo_id"])
        .agg(num_obs=("timestamp", "count"), num_unique_obs=("timestamp", "nunique"))
    )
    vol_count_ts = (
        volume_df.groupby(["publish_date", "geo_id"])
        .agg(num_obs=("timestamp", "count"), num_unique_obs=("timestamp", "nunique"))
    )
    ...
    assert (
        all(merged["_merge"] == "both") and
        all(merged.num_obs_x == merged.num_obs_y) and
        all(merged.num_unique_obs_x == merged.num_unique_obs_y)
    ), (
    ...
    volume_df = (
        add_max_ts_col(volume_df)[["geo_id", "publish_date", "val", "is_max_group_ts"]]
        .rename(columns={"val": "sample_size"})
    )
    ...
    # Generate stderr.
    df = df.assign(se=std_err(df)).drop(["_merge"], axis=1)
...
def add_max_ts_col(df):
    ...
    """
    assert_df = (
        df.groupby(["publish_date", "geo_id"])
        .agg(num_obs=("timestamp", "count"), num_unique_obs=("timestamp", "nunique"))
    )
    assert (
        all(assert_df.num_obs <= 2) and
        all(assert_df.num_obs == assert_df.num_unique_obs)
    ), (
    ...
    max_ts_by_group = (
        df.groupby(["publish_date", "geo_id"], as_index=False)["timestamp"]
        .max()

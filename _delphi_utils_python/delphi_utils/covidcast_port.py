...
Copied (almost exactly) from covidcast==0.2.2:

    https://github.com/cmu-delphi/covidcast/blob/c89e4d295550ba1540d64d2cc991badf63ad04e5/Python-packages/covidcast-py/covidcast/covidcast.py

The only changes made were:
- move NoDataWarning from errors.py to this file
- hardcode the version to 0.2.2
- format with black
"""
...
_HEADERS["user-agent"] = "covidcast/0.2.2 " + _HEADERS["user-agent"]
...
def signal(
    ...
    start_day = signal_meta["min_time"].to_pydatetime().date() if start_day is None else start_day

    end_day = signal_meta["max_time"].to_pydatetime().date() if end_day is None else end_day
    ...
    if _ASYNC_CALL:
        dfs = _async_fetch_epidata(
            data_source, signal, start_day, end_day, geo_type, geo_values, as_of, issues, lag, time_type
        )
    else:
        dfs = _fetch_epidata(
            data_source, signal, start_day, end_day, geo_type, geo_values, as_of, issues, lag, time_type
...
def metadata() -> pd.DataFrame:
    ...
    if meta["result"] != 1:
        ...
        raise RuntimeError("Error when fetching metadata from the API", meta["message"])
...
def aggregate_signals(signals: list, dt: list = None, join_type: str = "outer") -> pd.DataFrame:
    ...
    for i, (df, lag) in enumerate(zip(signals, dt)):
        ...
        if geo_type != first_geo_type:
            raise ValueError(
                "Multiple geo_types detected. " "All signals must have the same geo_type to be aggregated."
            )
        ...
        df_c.rename(
            columns={j: f"{source}_{sig_type}_{i}_{j}" for j in df_c.columns if j not in join_cols}, inplace=True
        )
        dt_dfs.append(df_c)
...
def _parse_datetimes(
    date_int: int, time_type: str, date_format: str = "%Y%m%d"
) -> Union[pd.Timestamp]:  # annotating nan errors
...
def _detect_metadata(
    data: pd.DataFrame, data_source_col: str = "data_source", signal_col: str = "signal", geo_type_col: str = "geo_type"
) -> Tuple:
...
def _fetch_epidata(
    ...
    while cur_day <= end_day:
        ...
        if day_data["message"] == "no results":
            warnings.warn(
                f"No {data_source} {signal} data found on {day_str} " f"for geography '{geo_type}'", NoDataWarning
            )
...
def _async_fetch_epidata(
    ...
    """
    warnings.warn(
        "`_async_fetch_epidata` is deprecated and will be removed in a future version.", category=DeprecationWarning
    )
    dfs = []
    params = []
    ...
    for day_data, params in output:
        if day_data["message"] == "no results":
            warnings.warn(
                f"No {data_source} {signal} data found on {params['time_values']} " f"for geography '{geo_type}'",
                NoDataWarning,
            )
        if day_data["message"] not in {"success", "no results"}:
            warnings.warn(
...
def _signal_metadata(data_source: str, signal: str, geo_type: str) -> dict:  # pylint: disable=W0621
    ...
    if matches.shape[0] == 0:
        raise ValueError(
            f"Unable to find metadata for source '{data_source}', " f"signal '{signal}', at '{geo_type}' resolution."
        )

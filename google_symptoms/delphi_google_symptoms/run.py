...
def run_module(params, logger=None):
    ...
    for geo_res in GEO_RESOLUTIONS:
        ...
        for metric, smoother in product(COMBINED_METRIC, SMOOTHERS):
            logger.info("Generating signal and exporting to CSV", geo_type=geo_res, metric=metric, smoother=smoother)
            df = df_pull
            ...
            if len(df) == 0:
                logger.info("No data for signal", geo_type=geo_res, metric=metric, sensor=sensor_name)
                continue

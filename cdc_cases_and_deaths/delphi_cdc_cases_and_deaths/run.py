...
when the module is run with `python -m delphi_cdc_cases_and_deaths`.
`run_module`'s lone argument should be a nested dictionary of parameters
loaded from the params.json file.  We expect the `params` to have the
following structure:
    ...
    - "indicator": (optional)
        - Any indicator-specific settings
"""
import time
...
from covidcast import covidcast_metadata
...
def run_module(params):
    ...
    ## Set export_start_date and export_end_date, based on user settings and metadata

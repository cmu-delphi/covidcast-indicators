...
class TestGeoMapper:
    ...
    def test_load_state_table(self):
        gmpr = GeoMapper()
        state_data = gmpr.load_crosswalk(from_code="state", to_code="state")
    ...
    def test_load_fips_msa_table(self):
        gmpr = GeoMapper()
        msa_data = gmpr.load_crosswalk(from_code="fips", to_code="msa")
        assert tuple(msa_data.columns) == ("fips", "msa")

    def test_load_jhu_uid_fips_table(self):
        gmpr = GeoMapper()
        jhu_data = gmpr.load_crosswalk(from_code="jhu_uid", to_code="fips")
        assert (jhu_data.groupby("jhu_uid").sum() == 1).all()[0]

    def test_load_zip_hrr_table(self):
        gmpr = GeoMapper()
        zip_data = gmpr.load_crosswalk(from_code="zip", to_code="hrr")
    ...
    def test_zip_to_hrr(self):
        ...
        assert np.allclose(new_data[['count','total']].sum(), self.zip_data[['count','total']].sum())
    ...
    def test_add_population_column(self):
        ...
        assert new_data["population"].sum() == 263790
        new_data = gmpr.add_population_column(self.zip_data, "zip")
        assert new_data["population"].sum() == 271528

...
from delphi_nowcast.deconvolution.deconvolution import (_linear_extrapolate, _construct_poly_interp_mat,
                                                        _impute_with_neighbors, _fft_convolve, _soft_thresh)

class Test_SoftThresh:

    def test__soft_thresh(self):
        np.testing.assert_array_equal(_soft_thresh(np.arange(-3,4), 1),
                                      np.array([-2, -1,  0,  0,  0,  1,  2]))

class Test_FFTConvolve:

    def test__fft_convolve(self):
        np.testing.assert_array_equal(_fft_convolve(np.array([1, 0, 1]), np.array([2, 7])),
                                      np.array([2, 7, 2]))

class Test_ImputeWithNeighbors:

    def test__impute_with_neighbors(self):
        np.testing.assert_array_equal(_impute_with_neighbors([np.nan, 1, np.nan, 3, np.nan]),
                                      np.array([1, 1, 2, 3, 3]))


    def test__impute_with_neighbors_no_missing(self):
        np.testing.assert_array_equal(_impute_with_neighbors(np.arange(5)),
                                      np.arange(5))

class Test_ConstructPolyInterpMat:

    def test__construct_poly_interp_mat(self):
        np.testing.assert_array_equal(_construct_poly_interp_mat(np.arange(6), 3),
                                      np.array([[3., -2.],
                                                [2., -1.],
                                                [1., 0.],
                                                [0., 1.],

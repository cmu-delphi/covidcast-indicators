...
from delphi_quidel_flutest.generate_sensor import (MIN_OBS, POOL_DAYS,
...
class TestGenerateSensor:
    def test_generate_sensor(self):
        ...
        assert (state_pct_positive.columns == ["geo_id", "val", "se", "sample_size", "timestamp"]).all()
        ...
        assert (state_test_per_device.columns == ["geo_id", "val", "se", "sample_size", "timestamp"]).all()
        ...
        assert (msa_pct_positive.columns == ["geo_id", "val", "se", "sample_size", "timestamp"]).all()
        ...
        assert (msa_test_per_device.columns == ["geo_id", "val", "se", "sample_size", "timestamp"]).all()

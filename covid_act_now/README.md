Covid Act Now (CAN) provides several testing metrics at the county and state level from various sources.
This indicator extracts only the county level PCR and specimen-based metrics sourced from the [CDC](https://covid.cdc.gov/covid-data-tracker/#county-view) and additionally aggregates them to state, MSA, HRR, HHS and national levels.
...
```
env/bin/python -m delphi_covid_act_now
```
...
```
(cd tests && ../env/bin/pytest <your_test>.py --cov=delphi_covid_act_now --cov-report=term-missing)
```

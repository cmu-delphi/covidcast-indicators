...
class TestCovidNet:
    ...
    def test_mappings(self):
        init_file = join(CACHE_DIR, "init.json")
    ...
    def test_hosp_data(self):
        ...
        # Non-parallel
        state_files = CovidNet.download_all_hosp_data(init_file, CACHE_DIR, parallel=False)
        assert len(state_files) == num_states
        for state_file in state_files:
            assert exists(state_file)

        # Parallel
        state_files_par = CovidNet.download_all_hosp_data(init_file, CACHE_DIR, parallel=True)

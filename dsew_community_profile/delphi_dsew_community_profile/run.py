...
def run_module(params):
    ...
    export_params = {k: pd.to_datetime(v) if v is not None else v for k, v in export_params.items()}
    ...
    for key, df in dfs.items():
        ...
        dates = create_export_csv(
            df, params["common"]["export_dir"], geo, make_signal_name(sig, is_prop), **export_params
        )
        if len(dates) > 0:
    ...
    if params["indicator"]["reports"] == "new":
        ...
        cpr_metadata = metadata[
            (metadata.data_source == "dsew-cpr") & (metadata.signal.isin(sensor_names.keys()))
        ]

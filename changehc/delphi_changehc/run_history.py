...
def run_module(params: Dict[str, Dict[str, Any]]):
    ...
    for drop_date in pd.date_range("2020-03-09","2021-11-30"):
        params["indicator"]["drop_date"] = drop_date.strftime("%Y-%m-%d")
        params["common"]["export_dir"] = \
            "./receiving/issue_%s/chng"%(drop_date.strftime("%Y%m%d"))

        try:
            os.mkdir("./receiving/issue_%s"%(drop_date.strftime("%Y%m%d")))
            os.mkdir("./receiving/issue_%s/chng"%(drop_date.strftime("%Y%m%d")))
        except:
            continue
        ...
        file_dict = {}
        file_dict["denom"] = "%s/%s_Counts_Products_Denom.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        file_dict["covid"] = "%s/%s_Counts_Products_Covid.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        file_dict["flu"] = "%s/%s_Counts_Products_Flu.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        file_dict["mixed"] = "%s/%s_Counts_Products_Mixed.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        file_dict["flu_like"] = "%s/%s_Counts_Products_Flu_Like.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        file_dict["covid_like"] = "%s/%s_Counts_Products_Covid_Like.dat.gz" % (params["indicator"]["input_cache_dir"],filedate)
        params["indicator"]["input_files"] = file_dict

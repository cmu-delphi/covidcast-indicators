...
class TestPull:
    ...
    def test_DatasetTimes_from_header(self):
        examples = [
            ...
            example(
                "TESTING: PREVIOUS WEEK (October 24-30, Test Volume October 20-26)",
                DatasetTimes("previous", date(2021, 10, 30), date(2021, 10, 26), None, None, None),
            ),
            example(
                "TESTING: LAST WEEK (October 24-November 30, Test Volume October 20-26)",
                DatasetTimes("last", date(2021, 11, 30), date(2021, 10, 26), None, None, None),
            ),
            example(
                "VIRAL (RT-PCR) LAB TESTING: LAST WEEK (June 7-13, Test Volume June 3-9 )",
                DatasetTimes("last", date(2021, 6, 13), date(2021, 6, 9), None, None, None),
            ),
            example(
                "VIRAL (RT-PCR) LAB TESTING: LAST WEEK (March 7-13)",
                DatasetTimes("last", date(2021, 3, 13), date(2021, 3, 13), None, None, None),
            ),
            example(
                "HOSPITAL UTILIZATION: LAST WEEK (June 2-8)",
                DatasetTimes("last", None, None, date(2021, 6, 8), None, None),
            ),
            example(
                "HOSPITAL UTILIZATION: LAST WEEK (June 28-July 8)",
                DatasetTimes("last", None, None, date(2021, 7, 8), None, None),
            ),
            example(
                "COVID-19 VACCINATION DATA: CUMULATIVE (January 25)",
                DatasetTimes("", None, None, None, date(2021, 1, 25), None),
            ),
            example(
                "COVID-19 VACCINATION DATA: LAST WEEK (January 25-31)",
    ...
    def test_Dataset_skip_overheader(self):
        examples = [
            ...
            example("HOSPITAL UTILIZATION: LAST WEEK (January 2-8)", False),
            example("HOSPITAL UTILIZATION: CHANGE FROM PREVIOUS WEEK", True),
            example("HOSPITAL UTILIZATION: DEMOGRAPHIC DATA", True),
            example("COVID-19 VACCINATION DATA: CUMULATIVE (January 25)", False),
            example("COVID-19 VACCINATION DATA: LAST WEEK (January 25-31)", False),
            example("COVID-19 VACCINATION DATA: DEMOGRAPHIC DATA", True),
        ]
        for ex in examples:
    ...
    def test_Dataset_retain_header(self):
        examples = [
            ...
            example("Confirmed COVID-19 admissions - last 7 days - age unknown", False),
            example("Confirmed COVID-19 admissions per 100 inpatient beds - last 7 days", False),
        ]
        for ex in examples:
    ...
    def test_fetch_listing(self, mock_listing, mock_exists):
        ...
        mock_listing.return_value.json = Mock(
            return_value={
                "metadata": {
                    "attachments": [
                        {"assetId": i.assetId, "filename": i.filename} for i in instances
                    ]
                }
            }
        )
    ...
    def test_generate_prop_signal_non_msa(self):
        ...
        geos = {
            "state": {"code_name": "state_id", "geo_names": ["pa", "wv"]},
            "county": {"code_name": "fips", "geo_names": ["36061", "06037"]},
            # nation uses the same logic path so no need to test separately
            "hhs": {"code_name": "hhs", "geo_names": ["1", "4"]},
        }
    ...
    def test_unify_testing_sigs(self):
        ...
        examples = [
            ...
            example(
                ...
                positivity_df.assign(sample_size=[100, 5, 1, 6])
                .iloc[[0, 3]]
                .assign(se=lambda df: np.sqrt(df.val * (1 - df.val) / df.sample_size)),
            ),
        ]
        for ex in examples:
    ...
    def test_interpolation(self):
        ...
        interpolated_dfs1 = interpolate_missing_values(
            {("src", "sig", False): pd.concat(missing_dfs)}
        )
        expected_dfs = pd.concat([sig1, sig2, sig3, sig4])
        _assert_frame_equal(
            interpolated_dfs1[("src", "sig", False)],
            expected_dfs,
            index_cols=["geo_id", "timestamp"],
        )

    @patch("delphi_dsew_community_profile.pull.INTERP_LENGTH", 2)
    def test_extend_listing(self):
        listing = [{"publish_date": date(2020, 1, 20) - timedelta(days=i)} for i in range(20)]
        examples = [
            # single range
            example(
            ...
            example(
                ...
                [
                    {"publish_date": date(2020, 1, 20)},
                    {"publish_date": date(2020, 1, 19)},
                    {"publish_date": date(2020, 1, 10)},
                    {"publish_date": date(2020, 1, 9)},
                ],
            ),
            # conjoined ranges
            example(

"""Tests for running the HHS Facilities indicator."""
import pandas as pd
import numpy as np
...
class TestGenerateSignals:
    """Tests for the `generate_signals` module."""

    def test_generate_signals(self):
        """Placeholder."""
        test_input = pd.DataFrame({"a": [1, 2, 3, 4, np.nan],
        ...
        expected = pd.DataFrame({"timestamp": [pd.Timestamp("20200131"),
                                               pd.Timestamp("20200201"),
                                               pd.Timestamp("20200202")],
                                 "geo_id": ["x", "x", "y"],
                                 "val": [5., 7., 10.],
